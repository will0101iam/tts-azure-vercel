...
const synthesize = async ({ text, rate, voice }) => {
  ...
  const resp = await fetch(ttsUrl, {
    ...
    headers: {
      ...
      "User-Agent": "tts-selection-vercel"
    },
    body: ssml
  })
  if (!resp.ok) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const audio = await synthesize({ text: cleanText, rate, voice })
    res.status(200)
    ...
    res.status(500).json({ error: String(error?.message || error || "Unknown error") })
  }
}
